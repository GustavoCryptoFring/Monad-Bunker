...
io.on('connection', (socket) => {
    ...
    card.usesLeft--;
    gameRoom.activeEffects[socket.id] = {}; // сброс старых эффектов
    ...
    switch (card.type) {
      ...
      case 'change':
        ...
        if (other) [me.actionCards, other.actionCards] = [other.actionCards, me.actionCards];
        break;
    }

    io.in('game-room').emit('action-card-used', {
      players:    gameRoom.players,
      playerName: me.name,                   // кто юзнул
      cardId:     card.id,
      cardName:   card.name,                 // какая карта
